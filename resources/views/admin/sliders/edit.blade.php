...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-gray-600 mt-1">Update slider content and settings</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title_en" value="{{ old('title_en', $slider->title_en) }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Title (Kannada)</label>
                    <input type="text" name="title_kn" value="{{ old('title_kn', $slider->title_kn) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <div>
                    ...
                    <textarea name="paragraph_en" rows="4" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('paragraph_en', $slider->paragraph_en) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Description (Kannada)</label>
                    <textarea name="paragraph_kn" rows="4"
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('paragraph_kn', $slider->paragraph_kn) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="button_text_en" value="{{ old('button_text_en', $slider->button_text_en) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Button Text (Kannada)</label>
                    <input type="text" name="button_text_kn" value="{{ old('button_text_kn', $slider->button_text_kn) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <select name="button_link_type" id="button_link_type" onchange="toggleLinkOptions()"
                    ...
                    <option value="major_development" {{ old('button_link_type', $slider->button_link_type) == 'major_development' ? 'selected' : '' }}>Major Development</option>
                    <option value="scam_exposed" {{ old('button_link_type', $slider->button_link_type) == 'scam_exposed' ? 'selected' : '' }}>Scam Exposed</option>
                    <option value="event" {{ old('button_link_type', $slider->button_link_type) == 'event' ? 'selected' : '' }}>Event</option>
                    <option value="custom" {{ old('button_link_type', $slider->button_link_type) == 'custom' ? 'selected' : '' }}>Custom URL</option>
                </select>
            </div>
            ...
            <div id="major_development_select" class="mt-4 hidden">
                ...
                <select name="button_link_id" class="major_development_id w-full px-4 py-2 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $item->id }}" {{ old('button_link_id', $slider->button_link_type == 'major_development' ? $slider->button_link_id : '') == $item->id ? 'selected' : '' }}>{{ $item->title_en }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div id="scam_exposed_select" class="mt-4 hidden">
                ...
                <select name="button_link_id" class="scam_exposed_id w-full px-4 py-2 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $item->id }}" {{ old('button_link_id', $slider->button_link_type == 'scam_exposed' ? $slider->button_link_id : '') == $item->id ? 'selected' : '' }}>{{ $item->title_en }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div id="event_select" class="mt-4 hidden">
                ...
                <select name="button_link_id" class="event_id w-full px-4 py-2 border border-gray-300 rounded-lg">
                    ...
                    <option value="{{ $item->id }}" {{ old('button_link_id', $slider->button_link_type == 'event' ? $slider->button_link_id : '') == $item->id ? 'selected' : '' }}>{{ $item->title_en }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div id="custom_url_input" class="mt-4 hidden">
                ...
                <input type="text" name="button_link_url" value="{{ old('button_link_url', $slider->button_link_url) }}" placeholder="https://..."
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <input type="hidden" name="media_id" id="media_id" value="{{ old('media_id', $slider->media_id) }}" required>
            
            <div id="selected-image" class="mb-4 {{ $slider->media_id || $slider->image ? '' : 'hidden' }}">
                <div class="relative inline-block">
                    <img id="image-preview" 
                         src="{{ $slider->media ? $slider->media->url : ($slider->image ? asset('storage/sliders/' . $slider->image) : '') }}" 
                         alt="Selected image" 
                         class="w-64 h-40 object-cover rounded-lg border-2 border-primary">
