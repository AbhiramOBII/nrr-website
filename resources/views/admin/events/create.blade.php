...
@section('page-description', 'Add a new event')
...
<form action="{{ route('admin.events.store') }}" method="POST" class="space-y-6" id="eventForm">
    @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            <div>
                ...
                <textarea name="short_description_en" rows="3" required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('short_description_en') }}</textarea>
            </div>
            <div>
                ...
                <textarea name="short_description_kn" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('short_description_kn') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="mb-6">
            ...
            <select name="featured_media_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                ...
                <option value="{{ $item->id }}">{{ $item->name }} ({{ $item->file_name }})</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 max-h-64 overflow-y-auto p-2 border border-gray-200 rounded-lg">
                ...
                <label class="relative cursor-pointer group">
                    <input type="checkbox" name="gallery_media[]" value="{{ $item->id }}" class="sr-only peer">
    ...
    <div class="flex justify-end gap-4">
        ...
        <button type="submit" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary/90">Create Event</button>
    </div>
</form>
@endsection
