...
@section('title', 'Edit Electronic Media')

@section('page-title', 'Edit Electronic Media')
@section('page-description', 'Update video details')
...
<form action="{{ route('admin.electronic-media.update', $electronicMedia) }}" method="POST" class="space-y-6" id="mediaForm">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title_en" value="{{ old('title_en', $electronicMedia->title_en) }}" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Title (Kannada)</label>
                <input type="text" name="title_kn" value="{{ old('title_kn', $electronicMedia->title_kn) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <input type="url" name="youtube_url" value="{{ old('youtube_url', $electronicMedia->youtube_url) }}" required
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
            <div>
                ...
                <textarea name="short_description_en" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('short_description_en', $electronicMedia->short_description_en) }}</textarea>
            </div>
            <div>
                ...
                <textarea name="short_description_kn" rows="3"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('short_description_kn', $electronicMedia->short_description_kn) }}</textarea>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-4">
            <div>
                ...
                <textarea name="description_en" id="description_en" rows="6"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('description_en', $electronicMedia->description_en) }}</textarea>
            </div>
            <div>
                ...
                <textarea name="description_kn" id="description_kn" rows="6"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg">{{ old('description_kn', $electronicMedia->description_kn) }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <input type="hidden" name="thumbnail_media_id" id="thumbnail_media_id" value="{{ old('thumbnail_media_id', $electronicMedia->thumbnail_media_id) }}">
        
        <div id="selected-thumbnail" class="mb-4 {{ $electronicMedia->thumbnailMedia ? '' : 'hidden' }}">
            <div class="relative inline-block">
                <img id="thumbnail-preview" src="{{ $electronicMedia->thumbnailMedia ? $electronicMedia->thumbnailMedia->url : '' }}" alt="Selected thumbnail" class="w-32 h-32 object-cover rounded-lg border-2 border-primary">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                    <option value="active" {{ $electronicMedia->status === 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ $electronicMedia->status === 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Sort Order</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $electronicMedia->sort_order) }}"
    ...
    <div class="flex justify-end gap-4">
        ...
        <button type="submit" class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary/90">Update Video</button>
    </div>
</form>
@endsection
